...
var msgs = map[peer.ID]int{}

// Calculates score for peer.
// In out case if someone sent more than 3 messages - we treating him as attacker.
// Here we don't use any timers or something else. It's just very simple example of `AppSpecificScore` usage.
func msgCountScore(id peer.ID) float64 {
	count, ok := msgs[id]
	if !ok {
		return 0
	}
...
func main() {
	...
	ps, err := pubsub.NewGossipSub(ctx, h, pubsub.WithPeerScore(
		&pubsub.PeerScoreParams{
			AppSpecificScore:       msgCountScore,
